...
I've created some documentation and some working code and I invite you to take a look at the documentation I wrote for it.  
...
System C is then extended to create System X (for exchange).  
...
where every step in the proof gets you closer to the goal.  
The exchange rule acts as a shortcut in a proof that would otherwise would require iteration (a step away from the goal).  
But finding exchanges has a cost, it requires finding cofactors in the confluence closure of an expression.  
